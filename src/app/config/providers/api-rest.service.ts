...
export class ApiRestService {
  ...
  getFrancois(): Observable<Toto[]> {
    return this.http.get('https://api.ichoui.fr/francois').pipe(
      // @ts-ignore
      map(data => data.images)
    );
  }
  ...
  getBanff(): Observable<Toto[]> {
    return this.http.get('https://api.ichoui.fr/banff').pipe(
      // @ts-ignore
      map(data => data.images)
    );
  }
  ...
  getJump(): Observable<Toto[]> {
    return this.http.get('https://api.ichoui.fr/jump').pipe(
      // @ts-ignore
      map(data => data.images)
    );
  }
